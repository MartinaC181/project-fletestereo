...
interface ServiceFormData {
  nombre: string;
  descripcion: string;
  caracteristicas: string; // Se maneja como string separado por \n
  precio: string;
}
...
export default function ServiciosPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (caracteristicasArray.length === 0) {
        ...
        setSubmitting(false); // Detener el envío
        return;
      }
      ...
      const serviceData = {
        ...
        activo: true // Por defecto se crea/actualiza como activo
      };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      // Usar deleteService (soft delete, pone activo=false)
      ...
      loadServices(); // Recargar la lista para mostrar el estado inactivo
    } catch (error) {
  ...
  const handleActivateService = async (service: Service) => {
    try {
      await servicesService.restoreService(service.id); // Llama al método para poner activo=true
      toast({
        title: "Éxito",
      ...
      <PageTransition>
        <div className="min-h-screen bg-background">
          ...
          <main className="pt-20">
            ...
            {/* Sección de Contenido (CRUD para Admin, Lista para Clientes) */}
            <section className="py-16 lg:py-24">
              <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                
                {/* Botón de Admin para Crear Servicio */}
                {isAdmin && (
                ...
                {loading ? (
                  ...
                  <div className="grid md:grid-cols-2 gap-8">
                    {services.map((service) => (
                      ...
                      >
                        {/* Badge de Inactivo (Solo Admin) */}
                        {isAdmin && !service.activo && (
                        ...
                        {/* Botones de Admin (Editar/Eliminar/Activar) */}
                        {isAdmin && (
                        ...
                        <CardHeader className="pb-4">
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 pr-20"> {/* Espacio para los botones de admin */}
            ...
            {/* Sección "Por qué elegirnos" (Pública) */}
          ...
          {/* Dialog para Crear/Editar Servicio (Solo Admin) */}
          ...
          {/* Dialog de Confirmación para Desactivar (Solo Admin) */}
